		...
		controller.addEventListener( 'connected', ( event ) => {
			...
			fetchProfile( xrInputSource, this.path, DEFAULT_PROFILE ).then( ( { profile, assetPath } ) => {
				...
				if ( cachedAsset ) {
					...
					null,
					( err ) => {
						console.error(`[SculptGL] GLTF Load Failed: ${controllerModel.motionController.assetUrl}`);
						if (window.screenLog) window.screenLog(`[GLTF Error] ${controllerModel.motionController.assetUrl}`, "red");
...
window._XRControllerModelFactory_utils = {
    fetchProfile: fetchProfile,
    MotionController: MotionController,
    addAssetSceneToControllerModel: addAssetSceneToControllerModel,
