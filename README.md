...
*   **v3.19.54**: **Full-body IK becomes a controller**: grab a bone with a hand or the mouse and the solver rearranges the rig around it, with every pin holding. Pins are now objects in the scene — selectable, gizmo-draggable, named after the bone they constrain, and saved with the file — so a pin's transform IS its anchor. Bones and pins are picked as points in a cone rather than by ray-vs-geometry, with pins taking priority, and a VR grab carries orientation as well as position so twisting your hand twists the limb. Plus rotation stored as Euler with real winding (a 3600-degree key now spins ten times), rotation and scale channels in the curve editor behind a T/R/S switch, IK pins held through keyframe playback, and orthographic mode that matches perspective on toggle.
*   **v3.19.23**: **Full-body IK that holds still**: posing a rig no longer jitters, pinned feet stay exactly on their pins, and bones stop winding up. The knee's bend direction is now chosen once per solve in closed form rather than clamped on every solver pass — clamping made the solver oscillate rather than settle, costing 40x the jitter and 270x the pin drift. Joint rotations are built from the pose you can see instead of accumulated frame by frame, which removes the twist ratchet behind the candy-wrapper collapse at the top of a thigh (a closed hip path used to leave 130 degrees of twist behind after four laps; it now leaves none). IK pins are held through keyframe playback and scrubbing, and the pin marker is drawn at the anchor with a dashed leader to the joint, so a pin the solve cannot reach shows the gap instead of hiding it. Plus a **Joints** display toggle for the joint markers and pins.
