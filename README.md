...
*   **v3.46.0**: **glTF import, materials, and the environment map that was never bound.** The app has exported glb since long before it could read one — there is an importer now, and the work is not the parsing but the three things glTF does not carry. **Quads**: FB_ngon_encoding restores the authored topology, but the *declaration* is not the test because Nomad does not declare it — a fan **chains**, and two triangles sharing a vertex and the edge between them are a quad. The camel arrives as **10,599 quads**. **Seams**: glTF splits the position array wherever UVs differ, so imported verbatim every seam is a crack; positions weld and the split stays in the uv corner indices. **A primitive is not an object** — one material per primitive turned a 6-object camel into 13, each copy carrying all the vertices and a third of the faces. Then materials: `COLOR_0` is a normalized ushort (read raw it clamps to white), `COLOR_1` is Nomad's per-vertex (roughness, metalness, mask) — the same vector this app uses — and **the UV pipeline was complete and simply never switched on** outside the two UV display modes. Albedo, metal/rough and **normal maps** all land, the last with no tangent attribute: the frame is rebuilt per pixel from derivatives, because a tangent goes stale the moment a brush moves a vertex. Underneath it all, **the IBL environment has been unbound since the Three.js port** — the mocked gl had no `TEXTURE0`, so the unit resolved to NaN and the bind found nothing; roughness and metalness had *no visible effect at all* (0.02 and 1.0 rendered identically). **Lights are objects you add and place**, locators like rig joints, with range taken from the scene. **Posing got faster twice**: a hidden mesh is no longer skinned (58.8ms → 19.0ms), and rigid parts can be parented rather than bound (1.52×). Plus the outliner resizes, holds its scroll and does Ctrl-toggle / Shift-range; Bind and Unbind both take the whole selection; transmissive materials read as glass; and the VR tool toast is no longer parked at the world origin on desktop.
*   **v3.45.0**: **The VR keyboard was mirrored, and the axis was the whole story.** Open a pinned panel's Save dialog and the keyboard came up reversed left to right. Every guard tested `scale.y < 0` — but three's `Matrix4.decompose` folds a negative determinant into **sx**, so a panel *placed by decomposing a matrix*, which is what pinning does, arrives as `(-1,1,1)` rather than `(1,-1,1)`. Same mirror, different axis, nothing fired. It was in **three** places — keyboard, numpad and confirm each carried their own copy — now one `matchPanelTransform`: decompose the source's world matrix, take its rotation and scale **signs**, no case analysis. **The numpad's offset was never clearance either**: it set an absolute distance from your head, which is not the same as standing off the panel's plane and is not monotonic in it — measured across pitch the gap ran 13.2 / 7.4 / 2.0 mm where it should be constant, so it went coplanar at some angles. "In front" means along the **normal**; the viewer is needed only for its sign. The keyboard had the same fault invisibly, so both use one `frontOfPanelOffset` (a flat 10.0 mm at every pitch). Plus **Cancel · Clear · Confirm** in their own row at the bottom of both overlays, same order and size, with the numpad keypad re-laid out around it; and **a modal no longer outlives its parent** — close the panel that summoned a keyboard, numpad or confirm and it closes too. Also from the user batch: **record starts on the first press** (the armed flag defaulted true before anyone had armed anything), and **"Bake Capsules" becomes "Bake Weight Cages"**.
