...
*   **v3.19.84**: **The animation editor learns about rigs**: a bone is now an ordinary keyable object. AutoKey keys the node you actually took (it asked which tool was active, and read a selection that is one gesture stale); a keyed bone gets its own dopesheet row instead of being folded into a summary row whose keys drew, highlighted, and could not be selected; the marquee reaches the first and last keys in time; and clicking a row or a key drives the graph editor, with the target row's name in yellow and the graph naming what it shows. Plus a capped lane height, one clamped scroll value shared by drawing and hit-testing, and a mouse-up that always releases the drag. Dopesheet keys also drop their per-kind colours for three states: white idle, yellow selected, cyan moving.
*   **v3.19.72**: **Rig ergonomics**: the VR transform gizmo reaches bones and pins (it had no pick at all), and a dragged bone poses the rig through the solver rather than editing bone length. A pinned child now outranks a driven twist — a 6DOF grab one bone above a pin dragged the foot off it by up to 1.5 units, while hips with pinned feet still twist. Plus thumbstick gizmo sizing, an optional 6DOF centre handle, **A** to pin the joint under the ray in Transform and Grab, a bound mesh locked out of viewport selection and saved that way, and bone capsules and weight colours off by default.
