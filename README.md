...
*   **v3.31.0**: **Apple Vision Pro: hands, menus and sculpting**: the headset could enter immersive and render, and nothing responded — four independent faults each producing that same symptom. A `gripSpace` gate skipped every visionOS hand (they arrive fully tracked with 25 joints and no gripSpace); a hand's `gamepad` is truthy with an EMPTY buttons array, which every `if (src.gamepad)` guard sailed past; the pinch test measured joint CENTRES, when two touching fingertips stay the sum of their radii apart; and three poses the controller grip only from `gripSpace`, so every wrist panel hung off the world origin. Plus the one that made menus unclickable: a legacy MiniHUD suppression killed the pinch within 25cm of the wrist — exactly where the wrist panel is clicked. Menus now come up on the wrist by default with panel-swap and Undo/Redo, gated to runtimes with hands and no buttons. New **Pinch distance** and **Grab speed** settings. AR remains Quest/GXR only — Apple confirmed at WWDC26 that visionOS Safari is still `immersive-vr` only.
*   **v3.30.194**: **Cast shadows in AR, and the object is the switch**: flag a proxy for the real table as a **Shadow Catcher** and it vanishes except for what the sculpt throws onto it — no enable toggle, no built-in floor, and the **Shadow Light** appears in the scene as an ordinary null you grab, animate and hide like any other object. A light here can only CAST (no sculpt shader reads a THREE.Light), so the controls are the light's position plus how dark and how soft. Alpha can only accumulate, so the shadow hides the real room instead of punching a hole in the model. Which meshes catch, and which object is the light, are saved in the rig footer block. Plus `scanPhantoms()`, which names every object actually drawing and the flag that governs it — and the two stray-marker bugs it found: a pin loaded from a file drew its pick sphere, and Hide All Decorations did not outrank the joint-dot exemptions.
