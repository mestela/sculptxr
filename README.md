...
*   **v1.0.40**: **Save Timestamps & Fit and Finish**: Date/timed filenames replace hardcoded `yourMesh` to prevent data loss. Scale-agnostic **Precision Center Dot** added to the brush circle for fine ray-alignment. Silenced HUD logs inside the main `VRMenu`. Menus scaled down slightly for better field of view.
